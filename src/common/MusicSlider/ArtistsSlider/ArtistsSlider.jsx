...
import Carousel from 'react-multi-carousel';
import 'react-multi-carousel/lib/styles.css';
import { useArtistsQuery } from '../../../hooks/useArtistQuery'; // 검색 쿼리 훅
import './ArtistsSlider.style.css';
...
const ArtistSlider = ({ title,searchQuery }) => {
  const { data: artists, isLoading, isError, error } = useArtistsQuery({ q: searchQuery });
      ...
      >
        {artists.map((artist) => (
          <div className="carousel-item" key={artist.id}>
            ...
            />
            <h2>{artist.name}</h2>
            <p>팔로워: {formatFollowers(artist.followers.total)}</p>
            <p>장르: {artist.genres.join(', ')}</p>
          </div>
        ))}
      </Carousel>
    </div>
